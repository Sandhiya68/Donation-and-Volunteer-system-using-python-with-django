...
from .forms import UserForm, DonorSignupForm, VolunteerSignupForm, LoginForm, MyPasswordChangedForm, DonateNowForm, DonationAreaForm
...
class signup_volunteer(View):
    ...
    def post(self,request):
        ...
        if form1.is_valid() & form2.is_valid():
            ...
            try:
                user = User.objects.create_user(first_name=fn,last_name=ln,username=us,email=em,password=pwd)
                Volunteer.objects.create(user = user , contact=contact, userpic=userpic,  idpic=idpic, 
                address=address, aboutme=aboutme, status='pending')
...
def donationrec_admin(request):
    if not request.user.is_authenticated:
        return redirect('/login-admin')
    donation = Donation.objects.filter(status='Dontion Received')
...
class changepwd_admin(View):
    def get(self,request):
        form = MyPasswordChangedForm(request.user) 
        return render(request, "changepwd-admin.html",locals())
    def post(self,request):
        form = MyPasswordChangedForm(request.user,request.POST)
        if not request.user.is_authenticated:
            return redirect('/login-admin')
...
class view_donationdetail(View):
    ...
    def post(self,request, pid):
        ...
        return render(request, "view-donationdetail", locals()) 
...
def index_volunteer(request):
    ...
    totalDonationDelivered = Donation.objects.filter(volunteer=volunteer,status="Donation Delivered").count()
...
def render_to_pdf(template_src, context_dict={}):
    ...
    result['Content-Disposition'] = 'attachment; filename="gallery_report.pdf"'
...
def download_pdf(request):
    ...
    return render_to_pdf('gallery_pdf_template.html', context)
