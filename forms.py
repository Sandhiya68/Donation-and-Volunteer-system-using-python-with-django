...
class LoginForm(AuthenticationForm):
    username = forms.CharField(required=True,widget=forms.TextInput(attrs={'autofocus':'True',
...
class MyPasswordChangedForm(PasswordChangeForm):
    ...
    {'autocomplete':'current-password','autofocus':True,'class':'form-control','placeholder':
     'Old Password'}))
...
class MySetPasswordForm(SetPasswordForm):
    new_password1 = forms.CharField(label="New Password", strip=False, widget=forms.PasswordInput(attrs=
    {'autocomplete':'new-password','class':'form-control'}),help_text=password_validation.
...
class DonationAreaForm(forms.ModelForm):
    class Meta:
        ...
        widgets={
            ...
            'description':forms.Textarea(attrs={'class':'form-control','plaeholder':'Description'}),
        }
        labels={
